...
namespace Grigori.Infrastructure.Indexing;
...
public class HnswIndex : IDisposable
{
    ...
    /// <param name="vectorProvider">Function to get all current vectors for persistence</param>
    public void Save(string indexPath, Func<IEnumerable<(long Id, float[] Embedding)>> vectorProvider)
    {
        lock (_lock)
        {
            if (_idMapping.Count == 0)
            {
            ...
            try
            {
                // Save ID mappings and embeddings for rebuild on load
                ...
                // Get all vectors
                var vectors = vectorProvider().ToDictionary(v => v.Id, v => v.Embedding);

                // Write ID mappings and embeddings
                writer.Write(_idMapping.Count);
                foreach (var id in _idMapping)
                {
                    writer.Write(id);
                    if (vectors.TryGetValue(id, out var embedding))
                    {
    ...
    /// <summary>
    /// Loads the HNSW index from disk and rebuilds it.
    /// </summary>
    ...
    public bool Load(string indexPath)
    {
        lock (_lock)
        {
            ...
            try
            {
                ...
                // Read ID mappings and embeddings
                var mappingCount = reader.ReadInt32();
                var vectors = new List<(long Id, float[] Embedding)>(mappingCount);
                ...
                {
                    BuildIndex(vectors);
                    _logger.LogInformation("Loaded and rebuilt HNSW index from {Path} ({Count} vectors)", indexPath, vectors.Count);
                    return true;
                }
